...
std::vector<YoloResults> AutoBackendOnnx::predict_once(const fs::path& imagePath, float& conf, float& iou, float& mask_threshold,
    ...
    int required_image_channels = this->getCh();//return 3;
...
/*
预处理：BGR→RGB（可选）、letterbox 缩放、转成 NCHW 的 blob，构造 ONNX Runtime 输入张量
推理：forward(inputTensors) 调 ONNX Runtime 推模型
后处理：根据 task_（SEGMENT / DETECT / POSE）调用不同的 postprocess_xxx
统计时间 & 打印：输出 preprocess / inference / postprocess 时间与结果数
返回：std::vector<YoloResults> 检测结果 / 关键点结果 / 分割结果
*/
