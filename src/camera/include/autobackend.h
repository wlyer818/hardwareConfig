...
struct YoloResults {
    ...
    cv::Rect_<float> bbox;            ///< The bounding box of the detected object. 可改成int
    cv::Mat mask;                     ///< The semantic segmentation mask (if available).语义分割掩码
    std::vector<float> keypoints{};   ///< Keypoints representing the object's pose (if available).表示对象姿势的关键点(如果可用)。
};
